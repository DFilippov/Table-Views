...
class EmojiTableViewController: UITableViewController {
    ...
    // при любом изменении будем перезаписывать файл, поэтому ставим обсервер didSet
    var emojis: [Emoji]! {
        didSet {
...
/* данный класс уже относится к протоколу UITableViewDataSource, тк в сториборде уже автоматом
 создался ( для первого и второго TableViews в Connections Inspector прописались в
 Referrencing Outlets dataSource)
 */
...
// MARK: - UITableViewDelegate (см аналогично выше)
extension EmojiTableViewController /*: UITableViewDelegate */ {
    /* после нажатия кнопки Edit слева от каждой ячейки появляются значки:
     (по умолчанию стоит перечисление .delete)
     .delete - красный кружок удалить
     .insert - зеленый куржок с плюсом - добавить
     .none - пусто (без значков)
     */
